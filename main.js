...
let currentApiKey = localStorage.getItem('arenamind_google_key') || null;
...
function sendChat() {
  ...
  // User message
  messages.innerHTML += `<div class="msg user"><div class="msg-bubble">${query}</div><div class="msg-time">${timeNow()}</div></div>`;
...
function placeOrder() {
  ...
  if (!seat) {
    ...
    confirm.textContent = '⚠ Please enter your seat number.';
    confirm.classList.remove('hidden');
    return;
  }
  ...
  confirm.innerHTML = `
    ...
    <p>Preparing items for Seat ${seat}. Total: ₹${total}</p>
  ...
  // Add alert
  alertLog.unshift({ type: 'info', msg: `📦 Order placed → Seat ${seat} (ETA ${eta} min)`, time: timeNow() });
  renderAlerts();
}
...
function saveApiKey() {
  ...
  if (key) {
    localStorage.setItem('arenamind_google_key', key);
    alert('API Key saved. Reloading map systems...');
    location.reload();
  } else {
...
function loadGoogleMaps() {
  ...
  // Final defensive check to prevent error overlays from showing if key is invalid
  const script = document.createElement('script');
  script.src = `https://maps.googleapis.com/maps/api/js?key=${currentApiKey}&callback=initMap&libraries=visualization`;
  script.async = true;
  script.defer = true;
  script.onerror = () => {
    console.warn('Map load failed. Falling back.');
    mapError();
  };
...
// ============================================================
function init() {
  // Expose key functions globally for inline HTML scripts
  ...
  const matchStatuses = ['Match in Progress', 'Half Time', 'Second Half', 'Injury Time', 'Match in Progress'];
  let msIdx = 0;
  setInterval(() => {
  ...
  // SVG map initial render (fallback)
